...
function member_history() {
    $("#member_add_edit").hide();
    $("#member_history").show();
    var id = $("#ID").val();
    var Name = $("#Name").val().replace(/ /g, "%20");
    var page = 0;
    $("#member_history").html("<img src='../../img/ajaxloader.gif'>").load("member_history_list.php?br=" + br + "&id=" + id + "&page=" + page + "&Name=" + Name);

}
function load_add() {
    $("#main-center-edit").html("<img src='../../img/ajaxloader.gif'>").load("main_center_detail.php?form=add", function () {
        initMap();
        $(".add,.view,.edit").css("display", "none");
...
function load_inside(form, id) {
    ...
    $('#main-center-edit').popup('open', function () {
    });
}
